...
Définissez l'attribut `clearable` pour` d-cascader` et une icône claire apparaîtra une fois sélectionnée et survolée
...
:::demo Tout comme `d-cascader`, vous pouvez définir des options alternatives par `options`, et activer d'autres fonctionnalités par `props`, voir le formulaire d'API ci-dessous pour plus de détails.
```html
