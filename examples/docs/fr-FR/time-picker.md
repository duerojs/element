...
:::demo Utilisez `d-time-select` puis assignez un horaire de début, de fin et un pas grâce aux propriétés `start`, `end` et `step` de l'attribut `picker-options`.
```html
<d-time-select
  v-model="value"
  :picker-options="{
...
:::demo Utilisez `d-time-picker` puis l'attribut `picker-options` et sa propriété `selectableRange` pour déterminer la tranche horaire disponible. Par défaut, vous pouvez utiliser la molette de votre souris, ou les flèches directionnelles si l'attribut `arrow-control` est présent.
...
| is-range | Si le mode intervalle est activé, marche avec `<d-time-picker>`. | boolean | — | false |
| arrow-control | Si les flèches directionnelles peuvent être utilisées, marche avec `<d-time-picker>`. | boolean | — | false |
...
| prefix-icon | Classe de l'icône de préfixe. | string | — | d-icon-time |
| clear-icon | Classe de l'icône d'effacement. | string | — | d-icon-circle-close |
