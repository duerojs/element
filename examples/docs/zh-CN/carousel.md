...
:::demo 结合使用`d-carousel`和`d-carousel-item`标签就得到了一个走马灯。幻灯片的内容是任意的，需要放在`d-carousel-item`标签中。默认情况下，在鼠标 hover 底部的指示器时就会触发切换。通过设置`trigger`属性为`click`，可以达到点击触发的效果。
```html
<template>
  <div class="block">
...
<template>
  <d-carousel height="200px" direction="vertical" :autoplay="false">
    <d-carousel-item v-for="item in 3" :key="item">
      <h3 class="medium">{{ item }}</h3>
    </d-carousel-item>
  </d-carousel>
</template>
...
| 方法名      | 说明          | 参数 |
|---------- |-------------- | -- |
| setActiveItem | 手动切换幻灯片 | 需要切换的幻灯片的索引，从 0 开始；或相应 `d-carousel-item` 的 `name` 属性值 |
| prev | 切换至上一张幻灯片 | — |
| next | 切换至下一张幻灯片 | — |
