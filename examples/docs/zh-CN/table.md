...
:::demo 当`d-table`元素中注入`data`对象数组后，在`d-table-column`中用`prop`属性来对应对象中的键名即可填入数据，用`label`属性来定义表格的列名。可以使用`width`属性来定义列宽。
```html
  <template>
    <d-table
      :data="tableData"
...
:::demo 只要在`d-table`元素中定义了`height`属性，即可实现固定表头的表格，而不需要额外的代码。
```html
<template>
  <d-table
    :data="tableData"
  ...
  <d-table
    ...
    <d-table-column
      ...
      <template slot-scope="scope">
        <d-button @click="handleClick(scope.row)" type="text" size="small">查看</d-button>
        <d-button type="text" size="small">编辑</d-button>
      </template>
    </d-table-column>
  </d-table>
</template>
...
:::demo 只需要在 d-table-column 里面嵌套 d-table-column，就可以实现多级表头。
```html
<template>
  <d-table
    :data="tableData"
    ...
    <d-table-column label="配送信息">
      <d-table-column
        prop="name"
        label="姓名"
        width="120">
      </d-table-column>
      <d-table-column label="地址">
        <d-table-column
          prop="province"
          label="省份"
          width="120">
...
:::demo Table 组件提供了单选的支持，只需要配置`highlight-current-row`属性即可实现单选。之后由`current-change`事件来管理选中时触发的事件，它会传入`currentRow`，`oldCurrentRow`。如果需要显示索引，可以增加一列`d-table-column`，设置`type`属性为`index`即可显示从 1 开始的索引号。
```html
<template>
  <d-table
    ref="singleTable"
  ...
  <div style="margin-top: 20px">
    <d-button @click="setCurrent(tableData[1])">选中第二行</d-button>
    <d-button @click="setCurrent()">取消选择</d-button>
  </div>
</template>
...
:::demo 实现多选非常简单: 手动添加一个`d-table-column`，设`type`属性为`selection`即可；默认情况下若内容过多会折行显示，若需要单行显示可以使用`show-overflow-tooltip`属性，它接受一个`Boolean`，为`true`时多余的内容会在 hover 时以 tooltip 的形式显示出来。
```html
<template>
  <d-table
    ref="multipleTable"
  ...
  <div style="margin-top: 20px">
    <d-button @click="toggleSelection([tableData[1], tableData[2]])">切换第二、第三行的选中状态</d-button>
    <d-button @click="toggleSelection()">取消选择</d-button>
  </div>
</template>
...
<template>
  <d-table
    ...
    <d-table-column label="操作">
      <template slot-scope="scope">
        <d-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">编辑</d-button>
        <d-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">删除</d-button>
      </template>
    </d-table-column>
  </d-table>
</template>
...
:::demo 通过设置 type="expand" 和 `Scoped slot` 可以开启展开行功能，`d-table-column` 的模板会被渲染成为展开行的内容，展开行可访问的属性与使用自定义列模板时的 `Scoped slot` 相同。
```html
<template>
  <d-table
    :data="tableData"
    ...
    <d-table-column type="expand">
      <template slot-scope="props">
        <d-form label-position="left" inline class="demo-table-expand">
          <d-form-item label="商品名称">
            <span>{{ props.row.name }}</span>
          </d-form-item>
          <d-form-item label="所属店铺">
            <span>{{ props.row.shop }}</span>
          </d-form-item>
          <d-form-item label="商品 ID">
            <span>{{ props.row.id }}</span>
          </d-form-item>
          <d-form-item label="店铺 ID">
            <span>{{ props.row.shopId }}</span>
          </d-form-item>
          <d-form-item label="商品分类">
            <span>{{ props.row.category }}</span>
          </d-form-item>
          <d-form-item label="店铺地址">
            <span>{{ props.row.address }}</span>
          </d-form-item>
          <d-form-item label="商品描述">
...
| resizable | 对应列是否可以通过拖动改变宽度（需要在 d-table 上设置 border 属性为真） | boolean | — | true |
