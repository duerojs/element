...
describe('Dialog', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" :visible="visible"></d-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.d-dialog__title').textContent).to.equal('dialog test');
  ...
  it('render correct content', done => {
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" :visible="visible">
            <span>这是一段信息</span>
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.d-dialog__footer .d-button');
      expect(vm.$el.querySelector('.d-dialog__body span').textContent).to.equal('这是一段信息');
  ...
  it('append to body', done => {
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" append-to-body :visible="visible"></d-dialog>
        </div>
      `,
  ...
  it('open and close', done => {
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" :visible.sync="visible">
            <span>这是一段信息</span>
          </d-dialog>
        </div>
      `,
  ...
  describe('props', () => {
    const getDialogVm = (props, options) => {
      return createVue(Object.assign({
        template: `
          <div>
            <d-dialog ${ props } :title="title" :visible="visible">
              <span>这是一段信息</span>
            </d-dialog>
          </div>
        `,
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.d-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.d-dialog').style.marginTop).to.equal('100px');
    });

    it('custom-class', () => {
      vm = getDialogVm('custom-class="my-dialog"');
      expect(vm.$el.querySelector('.d-dialog').classList.contains('my-dialog')).to.true;
    });
  });
  ...
  it('click header btn', done => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.d-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
  ...
  it('before close', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" :visible="visible" :before-close="beforeClose"></d-dialog>
        </div>
      `,
  ...
  it('destroyOnClose', async() => {
    vm = createVue({
      template: `
        <div>
          <d-dialog :title="title" :visible.sync="visible" destroy-on-close>
            <input />
          </d-dialog>
        </div>
      `,
