...
describe('Loading', () => {
  ...
  describe('as a directive', () => {
    it('create', done => {
      ...
      Vue.nextTick(() => {
        const mask = vm.$el.querySelector('.d-loading-mask');
    ...
    it('unbind', done => {
      ...
      Vue.nextTick(() => {
        ...
        Vue.nextTick(() => {
          ...
          vm2.show = false;
          Vue.nextTick(() => {
            expect(document.querySelector('.d-loading-mask')).to.not.exist;
            done();
          });
        });
      });
    ...
    it('body', done => {
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.d-loading-mask');
    ...
    it('lock', done => {
      ...
      Vue.nextTick(() => {
        ...
        vm.loading = false;
        document.body.removeChild(document.querySelector('.d-loading-mask'));
        document.body.removeChild(vm.$el);
        done();
      });
    });
    ...
    it('text', done => {
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.d-loading-mask');
        const text = mask.querySelector('.d-loading-text');
  ...
  describe('as a service', () => {
    it('create', () => {
      ...
      expect(document.querySelector('.d-loading-mask')).to.exist;
    });
    ...
    it('target', done => {
      ...
      let mask = document.querySelector('.d-loading-mask');
    ...
    it('fullscreen singleton', done => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          let masks = document.querySelectorAll('.d-loading-mask');
          expect(masks.length).to.equal(1);
          loadingInstance2.close();
          setTimeout(() => {
            masks = document.querySelectorAll('.d-loading-mask');
    ...
    it('text', () => {
      ...
      const text = document.querySelector('.d-loading-text');
      expect(text).to.exist;
      expect(text.textContent).to.equal('Loading...');
    });

    it('customClass', () => {
      loadingInstance = Loading({ customClass: 'd-loading-custom-class' });
      const customClass = document.querySelector('.d-loading-custom-class');
