...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.d-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.d-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'd-icon-check',
      inactiveIconClass: 'd-icon-close'
    });

    const icon = vm.$el.querySelector('.d-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <d-switch disabled v-model="value"></d-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.d-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <d-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></d-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('sets checkbox value', done => {
    vm = createVue({
      template: `
        <div>
          <d-switch v-model="value"></d-switch>
        </div>
      `,
      data() {
        return {
