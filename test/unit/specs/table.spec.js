...
describe('Table', () => {
  describe('rendering data is correct', () => {
    const vm = createVue({
      template: `
        <d-table :data="testData">
          <d-table-column prop="id" />
          <d-table-column prop="name" label="片名" />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" />
        </d-table>
      `,
    ...
    it('row length', () => {
      expect(vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr')).to.length(getTestData().length);
    });
  ...
  describe('attributes', () => {
    ...
    it('stripe', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.classList.contains('d-table--striped')).to.true;
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('border', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.classList.contains('d-table--border')).to.true;
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('fit', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.classList.contains('d-table--fit')).to.false;
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('show-header', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.querySelectorAll('.d-table__header-wrapper').length).to.equal(0);
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('tableRowStyle[Object]', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.querySelector('.d-table__body tr').style.height).to.equal('60px');
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('tableRowStyle[Function]', done => {
      ...
      setTimeout(_ => {
        let child1 = vm.$el.querySelector('.d-table__body tr:nth-child(1)');
        let child2 = vm.$el.querySelector('.d-table__body tr:nth-child(2)');
    ...
    it('current-row-key', done => {
      const vm = createVue({
        template: `
        <d-table :data="testData" row-key="id" highlight-current-row :current-row-key="currentRowKey">
          <d-table-column prop="name" label="片名" />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" />
      ...
      setTimeout(_ => {
        vm.currentRowKey = 1;
        const tr = vm.$el.querySelector('.d-table__body-wrapper tbody tr');
        setTimeout(_ => {
          expect(tr.classList.contains('current-row')).to.be.true;
          vm.currentRowKey = 2;

          const rows = vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr');
          setTimeout(_ => {
    ...
    it('select-on-indeterminate', done => {
      const vm = createVue({
        template: `
          <d-table :data="testData" @selection-change="change" :select-on-indeterminate="false" ref="table">
            <d-table-column type="selection" />
            <d-table-column prop="name" label="name" />
            <d-table-column prop="release" label="release" />
            <d-table-column prop="director" label="director" />
            <d-table-column prop="runtime" label="runtime" />
          </d-table>
        `,
      ...
      setTimeout(_ => {
        vm.$el.querySelector('.d-checkbox').click();
        setTimeout(_ => {
  ...
  describe('filter', () => {
    ...
    beforeEach(done => {
      vm = createVue({
        template: `
          <d-table ref="table" :data="testData" @filter-change="handleFilterChange">
            <d-table-column prop="name" label="片名" />
            <d-table-column prop="release" label="发行日期" />
            <d-table-column
              prop="director"
    ...
    it('render', () => {
      expect(vm.$el.querySelector('.d-table__column-filter-trigger')).to.exist;
    });

    it('click dropdown', done => {
      const btn = vm.$el.querySelector('.d-table__column-filter-trigger');
      triggerEvent(btn, 'click', true, false);
      setTimeout(_ => {
        const filter = document.body.querySelector('.d-table-filter');
    ...
    it('click filter', done => {
      ...
      setTimeout(_ => {
        ...
        // John Lasseter
        triggerEvent(filter.querySelector('.d-checkbox'), 'click', true, false);
        // confrim button
        setTimeout(_ => {
          triggerEvent(filter.querySelector('.d-table-filter__bottom button'), 'click', true, false);
          setTimeout(_ => {
            expect(vm.filters['director']).to.be.eql(['John Lasseter']);
            expect(vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr')).to.length(3);
            document.body.removeChild(filter);
            done();
          }, DELAY);
        }, 100);
    ...
    it('click reset', done => {
      ...
      setTimeout(_ => {
        ...
        setTimeout(_ => {
          // reset button
          triggerEvent(filter.querySelectorAll('.d-table-filter__bottom button')[1], 'click', true, false);
          setTimeout(_ => {
            expect(vm.filters['director']).to.be.eql([]);
            expect(filter.querySelector('.d-table-filter__bottom button').classList.contains('is-disabled')).to.true;
            document.body.removeChild(filter);
            destroyVM(vm);
            done();
          }, DELAY);
  ...
  describe('events', () => {
    const createTable = function(prop = '', opts) {
      return createVue({
        template: `
          <d-table :data="testData" @${prop}="handleEvent">
            <d-table-column type="selection" />
            <d-table-column prop="name" />
            <d-table-column prop="release" />
            <d-table-column prop="director" />
            <d-table-column prop="runtime"/>
          </d-table>
        `,
    ...
    it('select', done => {
      ...
      setTimeout(_ => {
        vm.$el.querySelectorAll('.d-checkbox')[1].click();
    ...
    it('cell-mouse-enter', done => {
      ...
      setTimeout(_ => {
        const cell = vm.$el.querySelectorAll('.d-table__body .cell')[2]; // first row
    ...
    it('cell-mouse-leave', done => {
      ...
      setTimeout(_ => {
        const cell = vm.$el.querySelectorAll('.d-table__body .cell')[7]; // second row
        const cell2 = vm.$el.querySelectorAll('.d-table__body .cell')[2]; // first row
    ...
    it('header-click', done => {
      ...
      setTimeout(_ => {
        const cell = vm.$el.querySelectorAll('.d-table__header th')[1]; // header[prop='name']
    ...
    it('sort-change', async() => {
      const vm = createVue({
        template: `
          <d-table ref="table" :data="testData" :default-sort = "{prop: 'runtime', order: 'ascending'}">
            <d-table-column prop="name" />
            <d-table-column prop="release" />
            <d-table-column prop="director" />
            <d-table-column prop="runtime" sortable/>
          </d-table>
        `,
  ...
  describe('column attributes', () => {
    const createTable = function(props1, props2, props3, props4, opts, tableProps) {
      return createVue(Object.assign({
        template: `
          <d-table :data="testData" ${tableProps || ''}>
            <d-table-column prop="name" ${props1 || ''} />
            <d-table-column prop="release" ${props2 || ''} />
            <d-table-column prop="director" ${props3 || ''} />
            <d-table-column prop="runtime" ${props4 || ''} />
          </d-table>
        `,
    ...
    it('width', done => {
      ...
      setTimeout(_ => {
        const ths = toArray(vm.$el.querySelectorAll('.d-table__header-wrapper col'))
    ...
    it('fixed', done => {
      ...
      setTimeout(_ => {
        expect(toArray(vm.$el.querySelectorAll('.d-table__fixed th:not(.is-hidden)'))
          .map(node => node.textContent))
          .to.eql(['test1', 'test3']);

        expect(toArray(vm.$el.querySelectorAll('.d-table__fixed-right th:not(.is-hidden)'))
          .map(node => node.textContent))
          .to.eql(['test2']);
        expect(vm.$el.querySelector('.d-table__body-wrapper').style.height).to.equal('');
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('formatter', done => {
      ...
      setTimeout(_ => {
        const cells = toArray(vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr td:first-child'));
    ...
    it('show-overflow-tooltip', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.querySelectorAll('.d-tooltip')).to.length(5);
        destroyVM(vm);
        done();
      }, DELAY);
    });
    ...
    it('render-header', done => {
      ...
      setTimeout(_ => {
        const headerCell = vm.$el.querySelector('.d-table__header-wrapper thead tr th:first-child .cell');
    ...
    it('selectable', done => {
      const vm = createVue({
        template: `
          <d-table :data="testData" @selection-change="change">
            <d-table-column type="selection" :selectable="filterSelect" />
            <d-table-column prop="name" label="name" />
            <d-table-column prop="release" label="release" />
            <d-table-column prop="director" label="director" />
            <d-table-column prop="runtime" label="runtime" />
    ...
    it('selectable === false & check selectAll status', done => {
      ...
      setTimeout(_ => {
        expect(vm.$el.querySelector('.d-checkbox').__vue__.checked).to.be.false;
        setTimeout(_ => {
          expect(vm.selected).to.length(0);
          destroyVM(vm);
          done();
    ...
    describe('type', () => {
      ...
      describe('= selection', () => {
        ...
        it('render', done => {
          setTimeout(_ => {
            expect(vm.$el.querySelectorAll('.d-checkbox')).to.length(getTestData().length + 1);
            done();
          }, DELAY);
        });
        ...
        it('select one', done => {
          ...
          setTimeout(_ => {
            vm2.$el.querySelectorAll('.d-checkbox')[1].click();
      ...
      describe('= index', () => {
        ...
        it('render', done => {
          setTimeout(_ => {
            expect(toArray(vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr td:first-child'))
      ...
      describe('= expand', () => {
        const createInstance = function(extra) {
          ...
          return createVue({
            template: `
            <d-table row-key="id" :data="testData" @expand-change="handleExpand" ${extra}>
        ...
        it('works', done => {
          ...
          setTimeout(_ => {
            expect(vm.$el.querySelectorAll('td.d-table__expand-column').length).to.equal(5);
            destroyVM(vm);
            done();
          }, DELAY);
        });
        ...
        it('should expand when click icon', done => {
          const vm = createInstance();
          setTimeout(_ => {
            vm.$el.querySelector('td.d-table__expand-column .d-table__expand-icon').click();
            setTimeout(_ => {
              expect(vm.$el.querySelectorAll('.d-table__expanded-cell').length).to.equal(1);
              expect(vm.expandCount).to.equal(1);
              vm.$el.querySelector('td.d-table__expand-column .d-table__expand-icon').click();
              setTimeout(_ => {
                expect(vm.$el.querySelectorAll('.d-table__expanded-cell').length).to.equal(0);
        ...
        it('should set expanded rows using expandRowKeys', done => {
          ...
          setTimeout(_ => {
            ...
            setTimeout(_ => {
              expect(vm.$el.querySelectorAll('.d-table__expanded-cell').length).to.equal(2);
              vm.expandRowKeys = [2];
              setTimeout(_ => {
                expect(vm.$el.querySelectorAll('.d-table__expanded-cell').length).to.equal(1);
                destroyVM(vm);
        ...
        it('should default-expand-all when default-expand-all is true', done => {
          ...
          setTimeout(_ => {
            expect(vm.$el.querySelectorAll('.d-table__expanded-cell').length).to.equal(5);
            destroyVM(vm);
            done();
          }, DELAY);
        });
    ...
    describe('sortable', () => {
      ...
      it('sortable orders', done => {
        ...
        setTimeout(_ => {
          ...
          setTimeout(_ => {
            const lastCells = vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr td:last-child');
  ...
  describe('summary row', () => {
    it('should render', done => {
      const vm = createVue({
        template: `
          <d-table :data="testData" show-summary>
            <d-table-column prop="name" />
            <d-table-column prop="release"/>
            <d-table-column prop="director"/>
            <d-table-column prop="runtime"/>
      ...
      setTimeout(_ => {
        const footer = vm.$el.querySelector('.d-table__footer');
    ...
    it('custom sum text', done => {
      const vm = createVue({
        template: `
          <d-table :data="testData" show-summary sum-text="Time">
            <d-table-column prop="name" />
            <d-table-column prop="release"/>
            <d-table-column prop="director"/>
            <d-table-column prop="runtime"/>
      ...
      setTimeout(_ => {
        const cells = toArray(vm.$el.querySelectorAll('.d-table__footer .cell'));
    ...
    it('custom summary method', done => {
      const vm = createVue({
        template: `
          <d-table :data="testData" show-summary :summary-method="getSummary">
            <d-table-column prop="name" />
            <d-table-column prop="release"/>
            <d-table-column prop="director"/>
            <d-table-column prop="runtime"/>
  ...
  describe('multi level column', () => {
    it('should works', done => {
      ...
      setTimeout(_ => {
        const trs = vm.$el.querySelectorAll('.d-table__header tr');
      ...
      const vm = createVue({
        template: `
          <d-table :data="testData">
            <d-table-column prop="name" />
            <d-table-column label="group">
              <d-table-column label="group's group">
                <d-table-column prop="release" />
                <d-table-column prop="runtime"/>
              </d-table-column>
              <d-table-column prop="director" />
  ...
  describe('dynamic column attribtes', () => {
    ...
    it('label', (done) => {
      const vm = createVue({
        template: `
          <d-table :data="testData">
            <d-table-column prop="name" :label="label"/>
            <d-table-column prop="release" />
            <d-table-column prop="director" />
            <d-table-column prop="runtime" />
          </d-table>
      ...
      setTimeout(() => {
        expect(vm.$el.querySelector('.d-table__header th .cell').textContent).to.equal('name');
        vm.label = 'NAME';
        vm.$nextTick(() => {
          expect(vm.$el.querySelector('.d-table__header th .cell').textContent).to.equal('NAME');
          destroyVM(vm);
    ...
    it('align', (done) => {
      const vm = createVue({
        template: `
          <d-table :data="testData">
            <d-table-column prop="name" :align="align"/>
          </d-table>
        `,
      ...
      setTimeout(() => {
        expect(vm.$el.querySelectorAll('.d-table__body td.is-right').length === 0).to.be.true;
        vm.align = 'right';
        vm.$nextTick(() => {
          expect(vm.$el.querySelectorAll('.d-table__body td.is-right').length > 0).to.be.true;
          destroyVM(vm);
    ...
    it('header-align', (done) => {
      const vm = createVue({
        template: `
           <d-table :data="testData">
            <d-table-column prop="name" :align="align" :header-align="headerAlign"/>
          </d-table>
        `,
      ...
      vm.$nextTick(() => {
        expect(vm.$el.querySelectorAll('.d-table__header th.is-left').length).to.above(0);
        expect(vm.$el.querySelectorAll('.d-table__header th.is-center').length).to.equal(0);
        expect(vm.$el.querySelectorAll('.d-table__header th.is-right').length).to.equal(0);
        vm.align = 'right';
        vm.$nextTick(() => {
          expect(vm.$el.querySelectorAll('.d-table__header th.is-left').length).to.equal(0);
          expect(vm.$el.querySelectorAll('.d-table__header th.is-center').length).to.equal(0);
          expect(vm.$el.querySelectorAll('.d-table__header th.is-right').length).to.above(0);
          vm.headerAlign = 'center';
          vm.$nextTick(() => {
            expect(vm.$el.querySelectorAll('.d-table__header th.is-left').length).to.equal(0);
            expect(vm.$el.querySelectorAll('.d-table__header th.is-center').length).to.above(0);
            expect(vm.$el.querySelectorAll('.d-table__header th.is-right').length).to.equal(0);
            vm.headerAlign = null;
            vm.$nextTick(() => {
              expect(vm.$el.querySelectorAll('.d-table__header th.is-left').length).to.equal(0);
    ...
    it('width', (done) => {
      const vm = createVue({
        template: `
          <d-table :data="testData" :fit="false">
            <d-table-column prop="name" :width="width"/>
          </d-table>
        `,
      ...
      setTimeout(() => {
        expect(vm.$el.querySelector('.d-table__body col').getAttribute('width')).to.equal('100');
        vm.width = 200;
        setTimeout(() => {
          expect(vm.$el.querySelector('.d-table__body col').getAttribute('width')).to.equal('200');
          destroyVM(vm);
          done();
        }, 100);
      }, DELAY);
    ...
    it('min-width', (done) => {
      const vm = createVue({
        template: `
          <d-table :data="testData" :fit="false">
            <d-table-column prop="name" :min-width="width"/>
          </d-table>
        `,
    ...
    it('fixed', (done) => {
      ...
      setTimeout(() => {
        expect(!vm.$el.querySelector('.d-table__fixed')).to.be.true;
        vm.fixed = true;
        setTimeout(() => {
          expect(!!vm.$el.querySelector('.d-table__fixed')).to.be.true;
          destroyVM(vm);
    ...
    it('prop', (done) => {
      ...
      setTimeout(() => {
        let firstColumnContent = vm.$el.querySelector('.d-table__body td .cell').textContent;
        let secondColumnContent = vm.$el.querySelector('.d-table__body td:nth-child(2) .cell').textContent;
        expect(firstColumnContent !== secondColumnContent).to.be.true;
        vm.prop = 'release';
        setTimeout(() => {
          firstColumnContent = vm.$el.querySelector('.d-table__body td .cell').textContent;
          secondColumnContent = vm.$el.querySelector('.d-table__body td:nth-child(2) .cell').textContent;
          expect(firstColumnContent === secondColumnContent).to.be.true;
          destroyVM(vm);
          done();
        }, 100);
  ...
  describe('methods', () => {
    const createTable = function(prop = '', opts) {
      return createVue({
        template: `
          <d-table ref="table" :data="testData" @${prop}="handleEvent">
            <d-table-column type="selection" />
            <d-table-column prop="name" />
            <d-table-column prop="release" />
            <d-table-column prop="director" />
    ...
    it('toggleAllSelection debounce', async() => {
      ...
      const vm = createVue({
        template: `
        <div>
          <d-table ref="table" :data="testData" @selection-change="change">
            <d-table-column type="selection" />
            <d-table-column prop="name" />
          </d-table>
          <d-table ref="table1" :data="testData1" @selection-change="change">
            <d-table-column type="selection" />
            <d-table-column prop="name" />
          </d-table>
        </div>
    ...
    it('sort correct change icon', async() => {
      ...
      const vm = createVue({
        template: `
          <d-table ref="table" :data="testData" >
            <d-table-column prop="name" sortable />
            <d-table-column prop="release" sortable />
            <d-table-column prop="director" sortable />
            <d-table-column prop="runtime" sortable />
          </d-table>
        `,
        data() {
    ...
    it('setCurrentRow', async() => {
      const vm = createVue({
        template: `
        <div>
          <d-table ref="table" :data="testData" highlight-current-row>
            <d-table-column prop="name" sortable />
            <d-table-column prop="release" sortable />
            <d-table-column prop="director" sortable />
            <d-table-column prop="runtime" sortable />
      ...
      await waitImmediate();
      const secondRow = vm.$el.querySelectorAll('.d-table__row')[1];
  ...
  it('hover', async() => {
    const vm = createVue({
      template: `
        <d-table :data="testData">
          <d-table-column prop="name" label="片名" fixed />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" />
        </d-table>
  ...
  it('highlight-current-row', done => {
    const vm = createVue({
      template: `
        <d-table :data="testData" highlight-current-row>
          <d-table-column prop="name" label="片名" />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" sortable />
        </d-table>
      `,
    ...
    setTimeout(_ => {
      ...
      setTimeout(_ => {
        ...
        setTimeout(_ => {
          ...
          const ths = vm.$el.querySelectorAll('.d-table__header-wrapper thead th');
  ...
  it('keep highlight row when data change', done => {
    const vm = createVue({
      template: `
        <d-table :data="testData" highlight-current-row row-key="release">
          <d-table-column prop="name" label="片名" />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" sortable />
    ...
    setTimeout(() => {
      let rows = vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr');
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          rows = vm.$el.querySelectorAll('.d-table__body-wrapper tbody tr');
          expect(rows[3].classList.contains('current-row')).to.be.true;
          destroyVM(vm);
          done();
        }, DELAY);
  ...
  it('keep highlight row after sort', done => {
    const vm = createVue({
      template: `
        <d-table :data="testData" row-key="release" highlight-current-row >
          <d-table-column prop="name" label="片名" />
          <d-table-column prop="release" label="发行日期" />
          <d-table-column prop="director" label="导演" />
          <d-table-column prop="runtime" label="时长（分）" sortable />
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        expect(rows[1].classList.contains('current-row')).to.be.true;
        const cells = vm.$el.querySelectorAll('.d-table__header-wrapper thead th > .cell');
  ...
  it('table append is visible in viewport if height is 100%', async() => {
    ...
    await waitImmediate();
    const emptyBlockEl = vm.$el.querySelector('.d-table__empty-block');
  ...
  describe('tree', () => {
    ...
    it('render tree structual data', async() => {
      vm = createVue({
        template: `
          <d-table :data="testData" row-key="release">
            <d-table-column prop="name" label="片名" />
            <d-table-column prop="release" label="发行日期" />
            <d-table-column prop="director" label="导演" />
            <d-table-column prop="runtime" label="时长（分）" />
      ...
      const rows = vm.$el.querySelectorAll('.d-table__row');
      expect(rows.length).to.equal(7);
      const childRows = vm.$el.querySelectorAll('.d-table__row--level-1');
      expect(childRows.length).to.equal(2);
      childRows.forEach(item => {
        expect(item.style.display).to.equal('none');
      });
      vm.$el.querySelector('.d-table__expand-icon').click();
    ...
    it('load substree row data', async() => {
      vm = createVue({
        template: `
          <d-table :data="testData" row-key="release" lazy :load="load">
            <d-table-column prop="name" label="片名" />
            <d-table-column prop="release" label="发行日期" />
            <d-table-column prop="director" label="导演" />
            <d-table-column prop="runtime" label="时长（分）" />
      ...
      const expandIcon = vm.$el.querySelector('.d-table__expand-icon');
      expandIcon.click();

      await waitImmediate();

      expect(expandIcon.classList.contains('d-table__expand-icon--expanded')).to.be.true;
      expect(vm.$el.querySelectorAll('.d-table__row').length).to.equal(8);
    });
    ...
    it('tree-props & default-expand-all & expand-change', async() => {
      ...
      await waitImmediate();
      const childRows = vm.$el.querySelectorAll('.d-table__row--levd-1');
      childRows.forEach(item => {
        expect(item.style.display).to.equal('');
      });
      const expandIcon = vm.$el.querySelector('.d-table__expand-icon');
    ...
    it('expand-row-keys & toggleRowExpansion', async() => {
      vm = createVue({
        template: `
          <d-table :data="testData" row-key="release" lazy :load="load" :expand-row-keys="['2003-5-30']" ref="table">
            <d-table-column prop="name" label="片名" />
            <d-table-column prop="release" label="发行日期" />
            <d-table-column prop="director" label="导演" />
            <d-table-column prop="runtime" label="时长（分）" />
      ...
      const expandIcon = childRows[0].querySelector('.d-table__expand-icon');
      expandIcon.click();
      ...
      await waitImmediate();
      expect(expandIcon.classList.contains('d-table__expand-icon--expanded')).to.be.false;
    });
  });
});
