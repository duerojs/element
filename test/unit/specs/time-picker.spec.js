...
describe('TimePicker', () => {
  ...
  it('set AM/PM format', done => {
    ...
    setTimeout(_ => {
      const list = vm.picker.$el.querySelectorAll('.d-time-spinner__list');
      const hoursEl = list[0];
      expect(hoursEl.querySelectorAll('.d-time-spinner__item')[0].textContent).to.equal('12 AM');
      expect(hoursEl.querySelectorAll('.d-time-spinner__item')[1].textContent).to.equal('01 AM');
      expect(hoursEl.querySelectorAll('.d-time-spinner__item')[12].textContent).to.equal('12 PM');
      expect(hoursEl.querySelectorAll('.d-time-spinner__item')[15].textContent).to.equal('03 PM');
      done();
    }, DELAY);
  });
  ...
  it('select time', done => {
    vm = createVue({
      template: '<d-time-picker ref="compo" v-model="value"></d-time-picker>',
      data() {
        return {
          value: ''
        };
    ...
    setTimeout(_ => {
      const list = timePicker.picker.$el.querySelectorAll('.d-time-spinner__list');

      const hoursEl = list[0];
      const minutesEl = list[1];
      const secondsEl = list[2];
      const hourEl = hoursEl.querySelectorAll('.d-time-spinner__item')[4];
      const minuteEl = minutesEl.querySelectorAll('.d-time-spinner__item')[36];
      const secondEl = secondsEl.querySelectorAll('.d-time-spinner__item')[20];
  ...
  it('click cancel button', done => {
    ...
    setTimeout(_ => {
      timePicker.picker.$el.querySelector('.d-time-panel__btn.cancel').click();
  ...
  it('click confirm button', done => {
    ...
    setTimeout(_ => {
      timePicker.picker.$el.querySelector('.d-time-panel__btn.confirm').click();
  ...
  it('set format', done => {
    ...
    setTimeout(_ => {
      expect(vm.picker.$el.querySelectorAll('.d-time-spinner__wrapper')[2].style.display).to.equal('none');
      done();
    }, 20);
  });
  ...
  it('selectableRange', done => {
    ...
    setTimeout(_ => {
      ...
      const minutesEl = list[1];
      hoursEl.querySelectorAll('.d-time-spinner__item')[18].click();
      setTimeout(_ => {
...
describe('TimePicker(range)', () => {
  ...
  it('create', done => {
    ...
    setTimeout(_ => {
      expect(vm.picker.$el.querySelectorAll('.d-time-range-picker__cell')).to.length(2);
  ...
  it('default value', done => {
    ...
    vm = createVue({
      template: '<d-time-picker ref="compo" is-range v-model="value" :default-value="defaultValue"></d-time-picker>',
      data() {
        return {
          value: '',
          defaultValue
  ...
  it('cancel button', done => {
    vm = createVue({
      template: '<d-time-picker ref="compo" is-range v-model="value"></d-time-picker>',
      data() {
        return {
          value: ''
        };
