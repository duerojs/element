...
describe('Transfer', () => {
  ...
  const createTransfer = (props, opts) => {
    return createVue(Object.assign({
      template: `
        <d-transfer :data="testData" ref="transfer" ${props}>
        </d-transfer>
      `,
  ...
  it('filterable', done => {
    ...
    const leftList = transfer.$el.querySelector('.d-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout(_ => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector('.d-transfer-panel__header .d-checkbox__label');
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(transfer.querySelector('.d-transfer-panel__list .d-checkbox__label span').innerText).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList = vm.$refs.transfer.$el.querySelector('.d-transfer-panel').__vue__;
  ...
  describe('target order', () => {
    it('original(default)', done => {
      ...
      setTimeout(() => {
        transfer.addToRight();
        setTimeout(() => {
          const targetItems = [].slice.call(vm.$el.querySelectorAll('.d-transfer__buttons + .d-transfer-panel .d-transfer-panel__body .d-checkbox__label span'));
